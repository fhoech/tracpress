...
function tracpress_milestone($atts = array(), $content = null) {
    ...
    $out .= '<meter class="meter" value="' . $closedposts . '" min="0" max="' . $openposts . '" low="0" high="' . $openposts . '" optimum="0">' . $closedposts . '/' . $openposts . '</meter><div class="tp-meter-details">' . $openposts . ' tickets (' . $closedposts . ' <a href="' . esc_url( site_url('/' . get_option('ticket_slug') . (!empty($term) ? '/' . $taxonomy . '/' . $term : '') . (!empty($version) ? '/version/' . $version : '') . '/status/closed' . (!empty($resolution) ? '/resolution/' . $resolution : '')) ) . '/">closed</a>, ' . ($openposts - $closedposts) . ' <a href="' . esc_url( site_url('/' . get_option('ticket_slug') . (!empty($term) ? '/' . $taxonomy . '/' . $term : '') . (!empty($version) ? '/version/' . $version : '') . '/status/!closed' . (!empty($resolution) ? '/resolution/' . $resolution : '')) ) . '/">open</a>)</div>';
    return $out;
}
